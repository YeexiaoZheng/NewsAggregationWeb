...
const newsGet = (db, url) => {
    common.request(url, (err, res, body) => {
        ...
        try {
            ...
            else news.author = eval(formats.author_format).split('责任编辑:').pop().replace('】', '').trim()
            if (news.author == '') news.author = formats.source
            // 摘要
            if (formats.summary_format == '') news.summary = news.title   // 没有摘要则使用title
            else news.summary = eval(formats.summary_format)
            if (news.summary) news.summary = news.summary.replace('\r\n', '').replace('\t', '')
            // 内容
            ...
            // console.log(news.url, news.source, news.encoding, news.title, news.keywords,
            //     news.author, news.summary, news.publishDate, news.crawlTime)
